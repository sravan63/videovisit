...
(function () {
    ...
    require([
            ...
            "utils/logger",
            "domReady",
            "vidyo/vidyo.client-1.0.1",
            "vidyo/vidyo.client.messages-1.0.1",
            "vidyo/vidyo.client.private.messages-1.0.1",
            "main.config",
            "handlebars",
            "bootstrap",
            "jquery.html5storage",
          ...
          applicationBuildCache = function () {
           ...
           self.cache.$inCallChatContainer = $(self.config.inCallChatContainer);
            ...
            applicationBuildSubscribeEvents = function () {
              ...
              self.events.configurationUpdateEvent = $({type:"configurationUpdateEvent"});
                /* Login state control */
            ...
            /* Fullscreen */
            self.events.fullscreenEvent  = $({type:"fullscreenEvent"});
         ...
         * @return {Object} Application object
         */
        var applicationBindEvents = function() {
            console.log('application::applicationBindEvents()');
            ...
            logger.log('log', 'application', 'applicationBindVidyoCallbacks()');
                  ...
                  */
                  OutEventJoining: function () {
                    ...
                    //if (e.result === "Failure") {
                    //    self.events.connectEvent.trigger('fail', "Failed to link endpoint");
                    //}
                },
                /**
               ...
               // },
                ...
                OutEventMouseDown: function(e) {
                    ...
                    console.log('OutEventMouseDown');
                    if (e.count === 2) {
                        if (self.isFullscreen) {
							uiFullscreenCancel();
                        } else {
                        	/* Commented by Ranjeet on 01/28/2014 to address defect DE732
                            ---*/
                            //uiFullscreenCancel();
                        }
                    }
                ...
                OutEventAddShare: function(e) {
                    ...
                    //console.log("OutEventAddShare: quitclick: " + quitclick + " endclick: " + endclick + " $('#meetingExpiredTimerFlag'): " + $('#meetingExpiredTimerFlag').val());
                    /* Added by Ranjeet on 01/14/2014 to address defect DE821
                    Start ---*/
                    //Check if Leave or End meeting or Meeting Expired Timer or Send email pop up is open
                    if(quitclick == 'false' && endclick == 'false' && $('#meetingExpiredTimerFlag').val() == 'false' && $('#sendEmailPopUpFlag').val() == 'false'){
                    	 self.cache.$plugin.css("width", "1px");
                         self.cache.$plugin.css("height", "1px");
						jNotify(
							'A participant has initiated desktop sharing.',
							{
							  ...
							  onClosed : function(){ // added in v2.0
								  console.log('add message closed');
								  self.cache.$plugin.css("width", "100%");
								  self.cache.$plugin.css("height", "100%");
							  },
                ...
                OutEventRemoveShare: function(e) {
                    ...
                    if(quitclick == 'false' && endclick == 'false' && $('#meetingExpiredTimerFlag').val() == 'false' && $('#sendEmailPopUpFlag').val() == 'false'){
                        ...
                        self.cache.$plugin.css("height", "1px");
                    	jNotify(
									'A participant has stopped desktop sharing.',
									{
									  autoHide : true, // added in v2.0
									  clickOverlay : false, // added in v2.0
									  MinWidth : 250,
									  ...
									  OpacityOverlay : 0.3,
									  onClosed : function(){ // added in v2.0
										  console.log('remove message closed');
										  self.cache.$plugin.css("width", "100%");
                                      	  self.cache.$plugin.css("height", "100%");
                                      	/*Added by Ranjeet on 01/27/2014 to address defect DE844 Start*/
                                      	if (self.smd == true && self.stopshare == false) {
                                            shares = clientSharesGet();
                                           // console.log("shares.numApp: " + shares.numApp + "Sharing Stopped: " + self.smd + " self.stopshare: " + self.stopshare + " self.currentShareId: " + self.currentShareId);
                                            if (shares.numApp <= 0 && self.currentShareId != null && self.currentShareId !== undefined) {
                                              //code
                                              console.log("Shared application was closed without clicking on the stop sharing button");
                                              self.smd = false;
                                              self.stopshare = true;
                                              self.currentShareId = undefined;
                                              self.events.shareUpdateEvent.trigger('done', shares);
                                              //console.log("shares.numApp: " + shares.numApp + "Sharing Stopped: " + self.smd + " self.stopshare: " + self.stopshare + " self.currentShareId: " + self.currentShareId);
                                              self.cache.$inCallButtonLocalShare.show();
                                            }
                                        }
                                      	/*---End---*/
									  },
                ...
                /**
                 * This is an out envent which gives special windows like kphc....we can use this event to trap this window.
                 * @param {Object} e Event details
                 */
                OutEventGetWindowsExt: function(e){
                    console.log("application::OutEventGetWindowsExt()", e);
                    //self.kphcShare = '';
	                for ( var i = 0; i < e.WindowName.length; i++)
	                {
	                    if ( (e.WindowName[i].toUpperCase().indexOf('HYPERSPACE - ') != -1) && (e.WindowName[i].toUpperCase().indexOf(' WITS ') != -1) )    
	                    {
	                        //alert('Setting kphcwindow ' + e.WindowName[i] + " " + e.windowID[i]);
	                         self.kphcShare = e.WindowName[i] + '|' + e.windowID[i];
	                        break;
	                    }
	                }
                ...
                OutEventConferenceEnded: function(e) {
                    console.log("application::OutEventConferenceEnded()", e);

					if ((isHost == 'false'||isHost == false) &&  quitclick == 'false' )
					{
						self.cache.$plugin.css("width", "1px");
	                    self.cache.$plugin.css("height", "1px");
						refreshMeetings = true; //In case of meeting ended by host, list of meetings for the join now page needs to be refreshed to remove the ended meeting.
						jNotify(
									'Your meeting has been disconnected. Please click on leave meeting.',
									{
									  autoHide : true, // added in v2.0
									  clickOverlay : false, // added in v2.0
									  MinWidth : 250,
            ...
            if(e.target.id=="inCallPluginAndControlsWrap" || e.target.id=="btnContainer") {
                if (isProvider != 'true' && self.cache.$inCallLocalShareList.is(":visible")){
                    if ($('#pluginContainer').css('visibility')=='hidden'){
                     ...
                     console.log('Making pluginContainer visible - SMD dropdown was open and user clicked on elsewhere');
                }
            }
        ...
        applicationBindSubscribeEvents = function() {
            ...
            self.events.connectEvent
                .on('done', function() {
                   ...
                   self.isJoining = false;
                    ...
                    if ( requestNum.numParticipants > 2 )
                    {
					        self.currentPreviewMode = "Dock";
                            clientPreviewModeSet("Dock");
					}else{
                   ...
                   /**if (self.config.enableChat && !self.cache.$inCallChatMinimizeLink.data("minimized")) {
                    ...
                    // alert("Current preview mode: " + self.currentPreviewMode);
                    ...
                    self.cache.$guestLoginPopup.modal('hide').promise()
                        .done(function() {
                            ...
                            uiLocalShareReset();
                            uiInCallShow(true, self.isFullpage);
            ...
            self.events.shareUpdateEvent
                .on('done', function(event, shares) {
                    ...
                    var transformedShares = { shares: [] };
					if ( shares.numApp > 0 && self.smd === false)
					{
						console.log('application::shareUpdateEvent::numApp > 0');
						self.cache.$inCallButtonLocalShare.hide();
					}
					else
						self.cache.$inCallButtonLocalShare.show();
                    for (var i = 0; i < shares.numApp; i++) {
                    ...
                    if ( self.currentShareId == '0000000000000001')
                    {
						//dont call clientShareSetCurrent if desktop
					}
					else
					{
                    	if (shares.numApp) {
        ...
        applicationBindUIEvents = function() {
            ...
            .click(function() {
                ...
                //alert(""+self.isMutedVideo);
               // if(self.isMutedVideo)self.cache.$inCallButtonMuteVideo.removeClass('btn btn-large btn-success').addClass('btn btn-large btn-failure');
                if(self.isMutedVideo){
                	self.cache.$inCallButtonMuteVideo.removeClass('btn btn-large btn-tmv-success').addClass('btn btn-large btn-tmv-failure');
                	self.cache.$inCallButtonMuteVideo.attr('title','Enable Video');
                }
                else{
                	self.cache.$inCallButtonMuteVideo.removeClass('btn btn-large btn-tmv-failure').addClass('btn btn-large btn-tmv-success');
                	self.cache.$inCallButtonMuteVideo.attr('title','Disable Video');
                }
                ...
                }else if (isProvider != 'true' && self.cache.$inCallLocalShareList.is(":visible")){
                    if ($('#pluginContainer').css('visibility')=='hidden'){
                        $('#pluginContainer').css('visibility', 'visible');
                     }
                }
        ...
        self.cache.$inCallButtonMuteMicrophone
          ...
          //Above 3 functions are updated by Srini on 09/18/2013
            ...
            self.cache.$inCallButtonLocalShare
                .click(function(e) {
                    ...
                    if ( isWindows)
                     var temp = clientLocalSharesGetExt();
                    /* Added by Ranjeet on 12/05/2013 to address US3423
                    Start ---*/
                    ...
                    if (isProvider=='true'){
                        ...
                        if (self.currentShareId == undefined) {
                            ...
                            $('#smdWarningModal').modal({'backdrop': 'static'});
                        }else{
                        	if (self.cache.$inCallLocalShareList.is(":visible")) {
                                self.cache.$inCallLocalShareList.hide();
                                if ($('#pluginContainer').css('visibility')=='hidden'){
                               ...
                               $('#smd_warning_button_continue').click();
                             }
                        }
                    }else{
                    	if (self.cache.$configurationContainer.is(":visible")) {
                ...
                .click(function() {
                    ...
                    if(isProvider == 'true'){
                        ...
                        $('#leave_meeting_button_no').click(function() {
                        	quitclick = 'false';
                            $('#dialog-block-override-meeting-leave').modal('hide');
                            $("#pluginContainer").css({"visibility":"visible"});
                        });

                        $('#leave_modal_cross_button').click(function() {
                        	quitclick = 'false';
                            $('#leave_meeting_button_no').click();
                        });
                    }
                    else{
                            $("#pluginContainer").css({"visibility":"hidden"});
                            $('#quitMeetingModal').modal({'backdrop': 'static'});
                            ...
                            $('#quit_meeting_button_no').click(function() {
                            	quitclick = 'false';
                                $('#quitMeetingModal').modal('hide');
                                $("#pluginContainer").css({"visibility":"visible"});
                            });

                            $('#quit_modal_cross_button').click(function() {
                            	quitclick = 'false';
                                $('#quit_meeting_button_no').click();
                            });

                    }

            });
          //Below code is added by Srini.P on 09/11/2013.

            self.cache.$inCallButtonEndMeeting.click(function() {
                    console.log("application::inCallButtonDisconnect::click");
                    endclick = 'true';
                    /* Added by Ranjeet on 12/05/2013 to address US3423
                    Start ---*/
                    if (isProvider=='true' && self.cache.$inCallLocalShareList.is(":visible")) {
                    ...
                    $("#pluginContainer").css({"visibility":"hidden"});
                    $('#dialog-block-override-meeting-end').modal({'backdrop': 'static'});
                    ...
                    $('#end_meeting_button_no').click(function() {
                    	endclick = 'false';
                        $('#dialog-block-override-meeting-end').modal('hide');
                        $("#pluginContainer").css({"visibility":"visible"});
                    });

                    $('#end_modal_cross_button').click(function() {
                    	endclick = 'false';
                        $('#end_meeting_button_no').click();
                    });
	        ...
	        uiParticipantsPromiseToSlide = function(show) {
		        ...
		        uiSharesUpdateWithData = function(data) {
		            ...
		            if (data.shares.length) {
		                self.cache.$inCallButtonShareBadge.show();
		                self.cache.$inCallButtonShareBadge.html("" + data.shares.length);
                ...
                uiInCallShow = function (show, inFullpage) {
                    ...
                    self.cache.$inCallPluginAndControls.css("width", "auto");
		               ...
		               //self.cache.$plugin.addClass("fullscreen");
		                self.cache.$pluginContainer.css("min-width", self.config.pluginWidth);
		                self.cache.$pluginContainer.css("min-height", self.config.pluginHeight);
		                self.cache.$plugin.css("width", "100%");
		                self.cache.$plugin.css("height", "100%");
		        ...
		        /**
		         * Set elem to full page
		         * @param  {Object} elem DOM element which to show in full page
		         * @return {Object} Application object
		         */
		        var uiFullPageToggle = function() {
		            console.log("application::uiFullPageToggle()");
		
		            self.isFullpage = !self.isFullpage;
		
		            if (self.isFullpage) {
		                uiInCallShow(true, true);
		                self.cache.$inCallButtonFullpage.removeClass("btn-inverse");
		                self.cache.$inCallButtonFullpage.addClass("btn-success");
		            }
		            else {
		                self.cache.$inCallButtonFullpage.addClass("btn-inverse");
		                self.cache.$inCallButtonFullpage.removeClass("btn-success");
		                uiInCallShow(true, false);
		            }
		        };
		        ...
		        clientLocalSharesGetExt = function() {
		           logger.log('info', 'call', 'clientLocalSharesGetExt()');
		           var inEvent;
		           var msg;
		
		            inEvent = vidyoClientMessages.inEventGetWindowsExt({
		                requestId: 1
		            });
		            if (self.client.sendEvent(inEvent)) {
		                msg = "VidyoWeb sent " + inEvent.type + " request successfully";
		            } else {
		                msg = "VidyoWeb did not send " + inEvent.type + " request successfully!";
		            }
		
		           logger.log('info', 'call', "clientLocalSharesGetExt(): " + msg);
		        ...
		        clientSharesSetCurrent = function(currApp) {
		           logger.log('info', 'call', 'clientSharesSetCurrent()');
		            ...
		            logger.log('info', 'call', "clientSharesSetCurrent() before: ", request);
		            ...
		            logger.log('info', 'call', "clientSharesSetCurrent() : " + msg, request);
		         ...
		         * @return {Object} Application object
		         */
		        var clientToggleLayout = function() {
		            console.log("application::clientToggleLayout()");
		            var params = {}, inEvent, msg;
		            params.numPreferred = self.numPreferred;
		            if (params.numPreferred == 0) {
		                params.numPreferred = 1;
		            } else { // 1
		                params.numPreferred = 0;
		            }
		            ...
		            console.log("application::clientToggleLayout(): " + msg);
			   ...
			   /**
				 ...
				 *  @return {Object} Application object
				 */
		        var clientSetLayout = function() {
		            console.log("application::clientSetLayout()");
		            var params = {}, inEvent, msg;
		            params.numPreferred = 1;
		            inEvent = vidyoClientMessages.inEventLayout(params);
		            if (self.client.sendEvent(inEvent)) {
		            ...
		            self.numPreferred = params.numPreferred;
		            console.log("application::clientSetLayout(): " + msg);
		      ...
		      /**
		        ...
		        clientParticipantsGet = function() {
					logger.log('info', 'call', 'clientParticipantsGet()');
						...
						var participants = [];
						var msg;
		                self.numParticipants = requestNum.numParticipants;
		        ...
		        vidyoPluginInitAndStart = function() {
		            ...
		            if (vidyoPluginIsLoaded()) {
				        ...
				        if (vidyoPluginIsInstalled()) {
						    ...
						    if (showPluginUpgradeMessage == 'true') {
								sessionStorage.setItem("isPluginUpgrade", "true");
								//code
								$('#pluginUpgradeModal').modal({'backdrop': 'static'});                        
			   ...
			   */
		       uiStartPluginDetection = function (runWhenDetected) {
		        ...
		        vidyoPluginLoad = function(config) {
		            ...
		            if (!self.client) {
				logger.log('info', 'plugin', 'vidyoPluginLoad() - failure');
		                return undefined;
		            }
		            return self;
		        };
