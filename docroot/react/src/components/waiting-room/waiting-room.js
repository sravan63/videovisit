...
class WaitingRoom extends React.Component {
        ...
        this.subscription = MessageService.getMessage().subscribe((message, data) => {
            switch(message.text) {
                ...
                case GlobalConfig.UPDATE_RUNNING_LATE:
                    this.setState({isRunningLate: message.data.isRunningLate,waitingroommsg: message.data.runningLatemsg,runningLateUpdatedTime:message.data.runLateMeetingTime});
                break;
            }
        });
    ...
    updateRunningLateTime(){
        return Utilities.formatInMeetingRunningLateTime(this.state.runningLateUpdatedTime);
    }
    handleResize(){
                ...
                <div className="full-waiting-room" style={{display: this.props.waitingroom.moreparticpants &&  window.innerWidth > 1024 ? 'none' : 'flex' }}>
    				<div className="conference-waiting-room">
    					<div className="waitingRoomMessageBlock row">
                            ...
                            <span className="waitingroom-text" style={{display: this.state.isRunningLate ? 'block' : 'none' }}>{translateLangText && translateLangText.WaitingRoomLabel}<b>{this.updateRunningLateTime()}</b>{translateLangText && translateLangText.Start}</span>
                ...
                <div className="half-waiting-room" style={{display: this.props.waitingroom.moreparticpants &&  window.innerWidth > 1024  ? 'block' : 'none' }}>
    				<div className="conference-waiting-room">
    					<div className="waitingRoomMessageBlock row">
                            ...
                            <span className="waitingroom-text" style={{display: this.state.isRunningLate  ? 'block' : 'none' }}>{translateLangText && translateLangText.WaitingRoomLabel}<b>{this.updateRunningLateTime()}</b></span>
                            <div className="runninglate-msg" style={{display: this.state.isRunningLate  ? 'block' : 'none' }}>{translateLangText && translateLangText.RunningLateLabel}</div>                                    
    					</div>
    				</div>
    			</div>                                  
