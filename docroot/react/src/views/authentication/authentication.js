                ...
                {this.state.isMobileError ?
                    (<div className="row error-text">
                        {this.state.tempAccessToken || this.state.isInApp ?
                           ...
                           :this.state.instantJoin ?(<p className="col-sm-12" style={{fontWeight:this.state.isBrowserBlockError ? 'bold': ''}}>{Details.errorCodes.ErrorInvalidLink}</p>):(<p className="col-sm-12">{Details.errorCodes.ErrorInvalidUSerID}</p>)
                        }
                    </div>)
                : ('')}
                {!this.state.isInApp ? (
